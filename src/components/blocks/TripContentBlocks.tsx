...
export function TripContentBlock(block: Options, context: BlockRenderContext) {
  ...
  if (block.variant === 'overview') {
    return (
      <div id="overview" className={styles.overview}>
        <div>{existingContent}{sections.map(section => <TripEditorialSection key={section.id ?? section.heading} section={section} />)}</div>
  ...
  return <>{existingContent}{sections.map((section, index) => <TripEditorialSection key={section.id ?? `${kind}-${index}`} section={section} variant={variant} id={index === 0 ? kind : `${kind}-${index}`} />)}</>
}
