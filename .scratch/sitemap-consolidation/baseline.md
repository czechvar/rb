...
The [single shareable redirect overview](legacy-redirect-overview.csv) covers all 216 old URLs: 75 approved exact replacements, 52 configured Location/Guide redirects, 7 temporary Event-to-category redirects, 37 temporary historical Date-to-category redirects, 10 approved Event-page redirects, 2 approved Event browse-index redirects, 8 broader `/trips` fallbacks, 24 existing same-path URLs, and one same-path CMS page. No inventoried URL lacks a target or matching configuration rule. `validationStatus` records the latest named-production check.
