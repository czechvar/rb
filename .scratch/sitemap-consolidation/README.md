...
The category and team decisions from 15 September 2026 are recorded in `mark-decisions.py` and in `.scratch/legacy-expiry-audit/all-old-url-inventory.csv` plus `.scratch/sitemap-redirect-mapping.csv`. The current Event and missing-Guide approval lists live in `src/lib/legacy-redirect-decisions.json`; Next redirects, parent eligibility, and the CSV generator read that same file. Re-running the decision script is idempotent.
