...
async function main() {
  ...
  assert((await page.getByText('Browser climbing trip', { exact: true }).count()) === 1)
  await page.getByRole('list', { name: 'Checkout progress' }).waitFor()
  await page.getByRole('listitem').filter({ hasText: 'Payment' }).getByText('Payment').waitFor()
  assert((await page.getByRole('button', { name: 'Edit payer address' }).count()) === 0)
  assert((await page.getByText('Order details', { exact: true }).count()) === 1)
  assert((await page.getByText('Payer details', { exact: true }).count()) === 1)
  assert((await page.locator('details[open]').count()) === 0)
  await page.screenshot({ path: path.join(output, 'payment-1440.png'), fullPage: true })
  passed('returning-owner-payment-keeps-action-primary-and-details-collapsed')
