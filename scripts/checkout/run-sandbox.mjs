...
async function main() {
  ...
  try {
    ...
    for (const [label, script] of [
      ...
      ['checkout', 'scripts/checkout/verify-sandbox.ts'],
    ]) {
      if (interrupted) throw new Error('interrupted')
      console.log(`checkout sandbox: ${label}`)
        ...
        child.on('close', (code) => {
          ...
          for (const line of output.split('\n')) {
            try {
              ...
              if (typeof d.checkFailed === 'string' && /^[a-z-]+$/.test(d.checkFailed)) console.log(JSON.stringify({checkFailed:d.checkFailed,errorKind:d.errorKind,fields:d.fields,knownError:d.knownError,sqlCode:d.sqlCode,relation:d.relation,errorName:d.errorName,frames:d.frames,typeErrorProperty:d.typeErrorProperty}))
              if (d.checkoutVerificationPassed === true) report.integrationPassed = true
            } catch {}
          }
    ...
    console.error('checkout sandbox failed; details suppressed')
    process.exitCode = 1
  } finally {
    if (created) {
      try {
        ...
        console.error('checkout sandbox: temporary database cleanup failed')
        process.exitCode = 1
      }
    }
    ...
    await mkdir(path.join(root, '.scratch/checkout-delivery'), { recursive: true })
    await writeFile(
      path.join(root, '.scratch/checkout-delivery/checkout-verification.json'),
...
main().catch(() => {
  console.error('checkout sandbox guard failed')
  process.exitCode = 1
})
