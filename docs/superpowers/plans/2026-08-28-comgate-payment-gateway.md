...
- **Terminal-state conflict, found by final review:** if an order is independently cancelled (e.g. the customer self-cancels a still-`pending` order in a second tab via `cancelMyOrderAction`) while a Comgate payment is in flight, and the webhook later reports `PAID`, `applyComgateWebhook`'s order-chain update hits `isTransitionAllowed('cancelled', 'paid') === false` and throws — the route 500s, Comgate retries, and every retry hits the same terminal-state rejection forever (no reconciliation path exists since `checkStatus` is deferred too). Solve this alongside the `checkStatus` cron and the `PaymentGatewayError`→4xx mapping item above rather than piecemeal — all three are about "what happens when a webhook can't be cleanly applied."
- `PaymentGatewayError` carries no HTTP status, so an uncaught one (bad merchant/secret, unhandled status, etc.) surfaces from the webhook route as a generic 500 — Comgate can't distinguish "stop retrying, this is malformed" from "transient, please retry," and monitoring can't distinguish expected rejections from real bugs. Not a data-safety issue (the throw happens before any write), but worth mapping `PaymentGatewayError` to an explicit 4xx at the webhook boundary in a fast-follow — flagged by Task 5's code review, cross-cuts the Task 3/4 contract so it wasn't fixed in-place.
