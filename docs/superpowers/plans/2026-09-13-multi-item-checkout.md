...
- Email-first checkout directs previous purchasers to password login. A signed-in verified account does not repeat email verification; an account without a purchase sends its first reservation directly to staff review.
- Signed-out first-time visitors supply name and email, with an optional phone number. A deliberate confirmation POST verifies their email and reserves the whole basket. Signed-in verified accounts skip that confirmation. No billing address or participant profiles are required at this stage.
