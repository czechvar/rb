...
| Order detail | `ReservationHeader` (eyebrow "Your order", title = authored trip name, status badge, "Order number" reference row). Two-column `layout`. Left `stack`: Participants panel, Your details (billing address) panel, payment-instructions `notice` when confirmed, Your note panel when present, "Cancel booking" `btn-ghost` when pending, "All orders" `btn-ghost`. Right `sidebar`: "Your trip" (`tripSummary`: name, dates, participant count) and "Order summary" with a single Total row; Subtotal and Discount rows render only when a discount exists; the VAT line stays as `muted` small print. |
