...
Public registration cannot assign an administrator role; role creation and changes are admin-only. Email verification is a deliberate POST for signed-out guests and holds no seats beforehand. A signed-in verified account does not repeat email verification: its first reservation goes directly to staff review, while returning purchasers continue to self-service reservation and payment. Verified new reservations and staff-approved guest reservations hold until staff cancellation. Returning unpaid reservations expire after 24 hours only after resolving payment uncertainty. Approval invites/links the verified email's account; a typed email never authenticates its owner. Paid customers use password login.
