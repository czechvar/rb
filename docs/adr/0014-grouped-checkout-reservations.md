...
The first complete payer address entered during checkout is stored on the Checkout, copied to its Orders, and saved as the account's default address when the account has no default. Later checkouts reuse that saved address without reopening an edit form in the payment journey; address changes remain an account-management concern.
