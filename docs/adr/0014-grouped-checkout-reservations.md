...
Public registration cannot assign an administrator role; role creation and changes are admin-only. A signed-out guest provides a name and email, while phone is optional; the registration form continues to require phone. Guest-created accounts therefore permit a missing phone until the customer supplies one. A signed-out guest receives a six-digit, short-lived email verification code and submits it inside the checkout flow. The code is stored only as a keyed hash, is single-use, and the verification POST performs the fresh availability check and reservation; no seats are held beforehand. The former verification-link pages remain for one release only to redeem already-issued links; new verification emails never link to them. A signed-in verified account does not repeat email verification, contact entry, saved billing entry, or staff review: the server reuses its account data, reserves for 24 hours and continues directly to payment. If no saved billing address exists, it is completed on the payment page. Purchase history still classifies the Checkout as new or returning but does not gate self-service payment. Verified guest reservations hold until staff cancellation and approval. Approval invitations remain fragment-token links that connect the verified email's account; a typed email never authenticates its owner. Creating an account from a valid approval invitation starts the authenticated session and continues directly to the approved Checkout's initial payment, which defaults to the amount due now. Paid customers use password login.
